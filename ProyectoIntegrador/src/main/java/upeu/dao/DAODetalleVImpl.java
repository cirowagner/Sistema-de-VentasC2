...
public class DAODetalleVImpl extends Conexion implements DAODetalleVenta{
    ...
    public List<DetalleVenta> listar() {
        ...
        try {
            PreparedStatement pstmt = conectar().prepareStatement("SELECT * FROM detalleVenta");
            ...
            while (rs.next()){
                ...
                dtll.setDescuento_DetalleV(rs.getInt(4));
                dtll.setPrecioTotal_DetalleV(rs.getDouble(5));
                dtll.setId_ProductoFK(rs.getInt(6));
                dtll.setId_VentaFK(rs.getInt(7));
                lista.add(dtll);
            }
    ...
    public void registrar(DetalleVenta dtll) {
        try {
            PreparedStatement pstmt = conectar().prepareStatement("INSERT INTO detalleVenta" +
                    " (CANTIDAD_Detalle, PrecioUnitario_Detalle, Descuento_Detalle, PrecioTotal_Detalle, ID_ProductoFK, ID_VentaFK)" +
                    " VALUES (?, ?, ?, ?, ?, ?)");
            ...
            pstmt.setInt(5, dtll.getId_ProductoFK());
            pstmt.setInt(6,dtll.getId_VentaFK());
            pstmt.executeUpdate();
        }catch (SQLException e){
            JOptionPane.showMessageDialog(null,"Error:: "+e.getMessage());
        }catch (Exception e){
